...
abstract class FirebaseDiscoverDataSource {
  ...
  Future<List<LookbookModel>> getAgentLookbooks(String agentId);
...
class FirebaseDiscoverDataSourceImpl implements FirebaseDiscoverDataSource {
  ...
  @override
  Future<List<LookbookModel>> getAgentLookbooks(String agentId) async {
    try {
      final QuerySnapshot snapshot = await _firestore
      ...
      return snapshot.docs.map((doc) {
        final data = doc.data() as Map<String, dynamic>;
        return LookbookModel.fromJson(doc.id, data);
      }).toList();
    } catch (e) {
      throw Exception('Failed to fetch agent lookbooks: $e');
    }
  }
