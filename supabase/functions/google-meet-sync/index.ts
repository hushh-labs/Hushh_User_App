...
serve(async (req)=>{
  ...
  try {
    ...
    // Handle OAuth callback FIRST (no auth required)
    if (pathname.includes('/callback')) {
      const supabaseClient = createClient(Deno.env.get('SUPABASE_URL') ?? '', Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? '');
      return handleOAuthCallback(req, supabaseClient);
    }
    
    // Initialize Supabase client for other endpoints (auth required)
    const supabaseClient = createClient(Deno.env.get('SUPABASE_URL') ?? '', Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? '');
...
function initiateOAuthFlow(userId) {
  ...
  const authUrl = `https://accounts.google.com/o/oauth2/v2/auth?` + `client_id=${clientId}&` + `redirect_uri=${encodeURIComponent(redirectUri)}&` + `response_type=code&` + `scope=${encodeURIComponent(scopes)}&` + `access_type=offline&` + `prompt=consent&` + `state=${userId}`;
  return new Response(JSON.stringify({
    success: true,
    authUrl: authUrl
  }), {
...
async function handleOAuthCallbackWithCode(userId, code, supabaseClient) {
...
async function handleOAuthCallback(req, supabaseClient) {
  const url = new URL(req.url);
  ...
  // Return HTML that communicates back to Flutter web app
...
<body>
  ...
  <script>
    (function() {
      console.log('OAuth callback - executing script');
      
      // Send success message to parent window (Flutter app)
      try {
        ...
        if (window.opener && !window.opener.closed) {
          ...
          console.log('No valid window.opener, this appears to be redirect mode');
          
          // Extract return URL from referrer or use current origin as fallback
          let returnUrl = document.referrer;
          if (!returnUrl || returnUrl.includes('accounts.google.com')) {
            // If no referrer or coming from Google, try to construct Flutter app URL
            returnUrl = window.location.origin;
          }
          
          // Remove any existing OAuth parameters and add our auth code
          try {
            const returnUri = new URL(returnUrl);
            returnUri.searchParams.delete('code');
            returnUri.searchParams.delete('state');
            returnUri.searchParams.delete('scope');
            returnUri.searchParams.set('code', '${code}');
            returnUri.searchParams.set('oauth_success', 'true');
            
            console.log('Redirecting to:', returnUri.toString());
            window.location.href = returnUri.toString();
          } catch (e) {
            console.error('Failed to redirect:', e);
            // Fallback: try to close after 3 seconds
            setTimeout(function() {
              try {
                window.close();
              } catch (e2) {
                console.log('Could not close window:', e2);
              }
            }, 3000);
          }
        }
...
async function storeGoogleMeetAccount(supabaseClient, userId, tokenData) {
  ...
  const { error } = await supabaseClient.from('google_meet_accounts').upsert(accountData, {
    onConflict: 'user_id'
  });
  if (error) {
...
async function checkConnectionStatus(userId, supabaseClient) {
  const { data, error } = await supabaseClient.from('google_meet_accounts').select('is_active, email').eq('user_id', userId).eq('is_active', true).single();
...
async function handleSyncRequest(userId, supabaseClient) {
  // Get stored access token
  const { data: account, error: accountError } = await supabaseClient.from('google_meet_accounts').select('access_token_encrypted, refresh_token_encrypted, token_expires_at').eq('user_id', userId).eq('is_active', true).single();
...
async function refreshAccessToken(refreshToken, supabaseClient, userId) {
  const clientId = Deno.env.get('GOOGLE_MEET_CLIENT_ID');
  const clientSecret = Deno.env.get('GOOGLE_MEET_CLIENT_SECRET');
  ...
  await supabaseClient.from('google_meet_accounts').update({
    access_token_encrypted: tokenData.access_token,
    token_expires_at: new Date(Date.now() + tokenData.expires_in * 1000).toISOString(),
    updated_at: new Date().toISOString()
  }).eq('user_id', userId);
...
async function fetchGoogleMeetData(accessToken) {
  ...
  const conferencesResponse = await fetch(`${baseUrl}/conferenceRecords?pageSize=50`, {
    headers
  });
  ...
  for (const conference of conferences){
    ...
    try {
      // Fetch participants
      const participantsResponse = await fetch(`${baseUrl}/conferenceRecords/${conferenceId}/participants`, {
        headers
      });
      ...
      // Fetch recordings
      const recordingsResponse = await fetch(`${baseUrl}/conferenceRecords/${conferenceId}/recordings`, {
        headers
      });
      ...
      // Fetch transcripts
      const transcriptsResponse = await fetch(`${baseUrl}/conferenceRecords/${conferenceId}/transcripts`, {
        headers
      });
...
async function storeGoogleMeetData(supabaseClient, userId, meetData) {
  console.log('💾 Storing Google Meet data in Supabase...');
  // Store conferences
  if (meetData.conferences.length > 0) {
    const conferencesToStore = meetData.conferences.map((conference)=>({
        user_id: userId,
        conference_name: conference.name,
        start_time: conference.startTime,
        end_time: conference.endTime,
        participant_count: 0,
        was_recorded: meetData.recordings.some((r)=>r.name.includes(conference.name.split('/').pop())),
        was_transcribed: meetData.transcripts.some((t)=>t.name.includes(conference.name.split('/').pop())),
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString()
      }));
    const { error: conferencesError } = await supabaseClient.from('google_meet_conferences').upsert(conferencesToStore, {
      onConflict: 'user_id,conference_name'
    });
    if (conferencesError) {
  ...
  if (meetData.participants.length > 0) {
    const participantsToStore = meetData.participants.map((participant)=>({
        user_id: userId,
        conference_id: null,
    ...
    const { error: participantsError } = await supabaseClient.from('google_meet_participants').upsert(participantsToStore);
  ...
  if (meetData.recordings.length > 0) {
    const recordingsToStore = meetData.recordings.map((recording)=>({
        user_id: userId,
        conference_id: null,
    ...
    const { error: recordingsError } = await supabaseClient.from('google_meet_recordings').upsert(recordingsToStore);
    if (recordingsError) {
  ...
  if (meetData.transcripts.length > 0) {
    const transcriptsToStore = meetData.transcripts.map((transcript)=>({
        user_id: userId,
        conference_id: null,
    ...
    const { error: transcriptsError } = await supabaseClient.from('google_meet_transcripts').upsert(transcriptsToStore);
    if (transcriptsError) {
...
async function fetchGoogleCalendarData(accessToken) {
  ...
  const eventsResponse = await fetch(`${baseUrl}/calendars/primary/events?` + `timeMin=${encodeURIComponent(timeMin)}&` + `timeMax=${encodeURIComponent(timeMax)}&` + `maxResults=250&` + `singleEvents=true&` + `orderBy=startTime`, {
    headers
  });
  ...
  for (const event of events){
    ...
    const hasMeetLink = event.hangoutLink || event.conferenceData?.entryPoints?.some((ep)=>ep.entryPointType === 'video' && ep.uri?.includes('meet.google.com')) || event.description && event.description.includes('meet.google.com');
    if (hasMeetLink) {
...
async function storeGoogleCalendarData(supabaseClient, userId, calendarData) {
  console.log('💾 Storing Google Calendar data in Supabase...');
  // Store calendar events
  if (calendarData.events.length > 0) {
    const eventsToStore = calendarData.events.map((event)=>{
      // Extract Google Meet link
      let googleMeetLink = event.hangoutLink;
      if (!googleMeetLink && event.conferenceData?.entryPoints) {
        const meetEntry = event.conferenceData.entryPoints.find((ep)=>ep.entryPointType === 'video' && ep.uri?.includes('meet.google.com'));
    ...
    const { error: eventsError } = await supabaseClient.from('google_calendar_events').upsert(eventsToStore, {
      onConflict: 'userId,google_event_id'
    });
    if (eventsError) {
  ...
  if (calendarData.attendees.length > 0) {
    const attendeesToStore = calendarData.attendees.map((attendee)=>({
        userId: userId,
        event_id: null,
    ...
    const { data: storedEvents } = await supabaseClient.from('google_calendar_events').select('id, google_event_id').eq('userId', userId);
    const eventIdMap = new Map();
    storedEvents?.forEach((event)=>{
      eventIdMap.set(event.google_event_id, event.id);
    });
    // Update attendees with correct event_id
    const finalAttendeesToStore = attendeesToStore.filter((attendee)=>eventIdMap.has(attendee.google_event_id)).map((attendee)=>({
        ...attendee,
    ...
    if (finalAttendeesToStore.length > 0) {
      const { error: attendeesError } = await supabaseClient.from('google_calendar_attendees').upsert(finalAttendeesToStore);
      if (attendeesError) {
...
async function correlateCalendarWithMeetings(supabaseClient, userId) {
  console.log('🔗 Correlating calendar events with Google Meet conferences...');
  // Get calendar events with Google Meet links
  const { data: calendarEvents } = await supabaseClient.from('google_calendar_events').select('id, google_meet_link, start_time, end_time, summary').eq('userId', userId).not('google_meet_link', 'is', null);
  // Get Google Meet conferences
  const { data: meetConferences } = await supabaseClient.from('google_meet_conferences').select('id, conference_name, start_time, end_time').eq('user_id', userId);
  ...
  if (correlations.length > 0) {
    const { error: correlationError } = await supabaseClient.from('google_meet_calendar_links').upsert(correlations, {
      onConflict: 'calendar_event_id,meet_conference_id'
    });
    if (correlationError) {
